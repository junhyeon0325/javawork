...
<%
 /**
  * @Class Name : TodoRegister.jsp
  * @Description : Todo Register 화면
...
<c:set var="registerFlag" value="${empty todoVO.no ? '등록' : '수정'}"/>
...
function fn_egov_selectList() {
   	document.getElementById("detailForm").action = "<c:url value='/todo/TodoList.do'/>";
   	document.getElementById("detailForm").submit();		
}

/* 글 삭제 function */
function fn_egov_delete() {
   	document.getElementById("detailForm").action = "<c:url value='/todo/deleteTodo.do'/>";
   	document.getElementById("detailForm").submit();		
}
...
function fn_egov_save() {	
	...
	/* TODO Validation기능 보완 */
  	...
  	frm.action = "<c:url value="${registerFlag == '등록' ? '/todo/addTodo.do' : '/todo/updateTodo.do'}"/>";
    frm.submit();
...
<form:form modelAttribute="todoVO" name="detailForm" id="detailForm" >
<div id="content_pop">
	<!-- 타이틀 -->
	<div id="title">
		<ul>
	...
	<table width="100%" border="1" cellpadding="0" cellspacing="0" >
	   ...
	   <tr>
		...
		<tr>
			<th>COMPLETE</th>
			<td>
				<form:input path="complete" cssClass="txt"/>
				&nbsp;<form:errors path="complete" />
			</td>
		</tr>	
	</table>
  </div>
