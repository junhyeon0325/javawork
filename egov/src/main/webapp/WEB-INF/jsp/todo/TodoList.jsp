...
<%
 /**
  * @Class Name : TodoList.jsp
  * @Description : Todo List 화면
...
function fn_egov_select(no) {
	document.getElementById("listForm").no.value = no;
   	document.getElementById("listForm").action = "<c:url value='/todo/updateTodoView.do'/>";
   	document.getElementById("listForm").submit();
}

/* 글 등록 화면 function */
function fn_egov_addView() {
   	document.getElementById("listForm").action = "<c:url value='/todo/addTodoView.do'/>";
   	document.getElementById("listForm").submit();		
}
...
function fn_egov_link_page(pageNo){
	...
	document.getElementById("listForm").action = "<c:url value='/todo/TodoList.do'/>";
   	document.getElementById("listForm").submit();
}
	...
	<div id="table">
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
						...
						<td align="center" class="listtd"><c:out value="${result.complete}"/>&nbsp;</td>
				    			</tr>
			</c:forEach>
		</table>
	</div>
